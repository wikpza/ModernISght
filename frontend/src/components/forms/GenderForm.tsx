...
type GenderFormData = z.infer<typeof formSchema>

type Props = {
    onSave: (GenderFormData: Gender)=>void,
...
const GenderForm = ({onSave, setOpen,type="POST", gender=
    {
        name:"",
        _id:''
    }
    ...
    const onSubmit = (formDataJson: Gender) => {
        if(gender && type === 'PATCH'){
            formDataJson._id = gender._id
        }
        onSave(formDataJson)
    ...
    return (
        <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className={'rounded-lg md:p-5 space-y-4 text-gray-700 '}>
                ...
                <CustomInput name={'name'} label={'Name of the Gender'} form={form}/>
