...
import {useCreateGender} from "../../../../../api/GenderAPI.tsx";
...
export default () => {
    ...
    const {createGender, isLoading, isSuccess} = useCreateGender()
                    ...
                    <Dialog.Content className="DialogContent rounded sm:rounded-t rounded-t-3xl w-full ">
                        <DialogHeader>
                            <div className={'flex flex-row justify-between'}>
                                <DialogTitle className={'text-2xl mb-2 '}>Add new Gender</DialogTitle>
                                <DialogClose> <AiOutlineClose className={' w-8 h-8 bg-gray-200'} /></DialogClose>
                            </div>

                            <Separator className={'bg-slate-400 w-full h-[1px] '}/>

                            <GenderForm onSave={createGender} isLoading={isLoading} setOpen={setOpen}/>
