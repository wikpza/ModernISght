...
import {useAddCart, useDeleteCart} from "../../../../api/CartAPI.tsx";
...
const CartItem = ({product}:Props) => {
    ...
    const [quantityNumber, setQuantityNumber] = useState<number>(product.quantity)
    // const {} =

    function validateNumber(input: number, secondNumber: number): boolean {
        // Проверяем, что число от 1 до 10 (включительно)
        if (input < 1 ) {
        ...
        if ( input > 10) {
            toast.error("quantity can not be more than 10")
            return false;
        }


        if (input > secondNumber) {
            toast.error(`only ${secondNumber} left in stock.`)
            return false;
        }
        ...
        addCart(
            {
                productId:product.product.productVariant._id,
    ...
    const handleChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!isNaN(numericValue)) {
            setQuantityNumber(numericValue); // Если введено корректное число, обновляем состояние
        }

    };
    const navigate = useNavigate()
    const {deleteCart, isSuccess} = useDeleteCart()
    const {addCart, isSuccess:isAddCartLoading} = useAddCart()

    if(isSuccess ||  isAddCartLoading) window.location.reload()
    return (
               ...
               <section className={'w-2/5'}
                ...
                <section className={'space-y-2 w-3/5'}>
                    ...
                    <div className={'flex justify-between items-center mb-4'}>
                        ...
                        <div className={' border-gray-500 border rounded p-2'}
                        onClick={()=>deleteCart({productId:product.product.productVariant._id, sizeId:product.size._id})}>
                    ...
                    {(product.inventoryQuantity === 0) ?
                        ...
                        :
                        <div className={'text-sm '}>quantity : <span className={'text-red-700'}>{product.inventoryQuantity <= 10 ?`only ${product.inventoryQuantity} in stock`:""}</span>
                            <div className={'flex'}>
                                <Input value={quantityNumber} type={"number"} onChange={handleChange}/>
                                <Button className={'bg-blue-700 hover:bg-blue-800'}
                                        onClick={() => validateNumber(quantityNumber, product.inventoryQuantity)}
                                >save</Button>
                            </div>
                        </div>
                    }
